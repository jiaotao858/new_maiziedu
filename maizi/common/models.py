...
class UserProfile(AbstractBaseUser,PermissionsMixin):
    ...
    avatar_url = models.ImageField(upload_to='media/avatar/%Y/%m', default='avatar/default_big.png', max_length=200, blank=True, null=True, verbose_name=u'头像220x220')
    avatar_middle_thumbnall = models.ImageField(upload_to='media/avatar/%Y/%m', default='avatar/default_middle.png', max_length=200, blank=True, null=True, verbose_name=u'头像104x104')
    avatar_small_thumbnall = models.ImageField(upload_to='media/avatar/%Y/%m', default='avatar/default_small.png', max_length=200, blank=True, null=True, verbose_name=u'头像70x70')
    ...
    # 是否是老师
    def is_teacher(self):
        if self.groups.filter(name='teacher').count() > 0:
            return True
        return False

    # 是否是学生
    def is_student(self):
        if self.groups.filter(name='student').count() > 0:
            return True
        return False

    # 跳转至对用教师页
    def get_absolute_url(self):
        return reverse('common:teacher', kwargs={'pk': self.pk})
